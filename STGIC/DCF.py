...
def dilate2_train5_stereoseq(model3,model2,coord_x,coord_y,n_clusters,step_kl,step_con,step_con1,step_ce,label,image,mask,y_pred_init,max_pretrain_epoch,max_epoch=200,update_interval=3,tol=1e-3,q_cut=0.5,cat_tol=1e-2,mod3_ratio=0.7,pretrain_lr=0.005,lr=0.001,device=torch.device('cuda:0')):
...
def dilate2_train5_visium(model3,model2,coord_x,coord_y,n_clusters,step_kl,step_con,step_con1,step_ce,label,image,mask,y_pred_init,max_pretrain_epoch,max_epoch=200,update_interval=3,tol=1e-3,q_cut=0.5,cat_tol=1e-2,mod3_ratio=0.7,pretrain_lr=0.05,lr=0.01,device=torch.device('cuda:0')):
    mask_tensor = torch.from_numpy(mask).view(-1).to(device)
    data = image.transpose((2, 0, 1)) 
    data = data[np.newaxis, :, :, :]
    data = torch.Tensor(data).to(device)
